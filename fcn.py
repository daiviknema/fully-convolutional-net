...
class FCN(object):
  def __init__(self, trainval_root_dir, test_root_dir,
               vgg_params_path, log_lvl=logging.DEBUG):
    # Disable TF logging
    ...
    # Init dataset reader
    self.dataset_reader = VOCDataReader(trainval_root_dir, test_root_dir,
                                        logging.INFO)
  ...
  def _get_vgg_conv_params(self, name):
    ...
    if name in layers_common_with_vgg:
      dic = {
        'weights': tf.Variable(
            self.vgg_params[name]['weights'],
            '{}_weights'.format(name)
            ),
        'biases': tf.Variable(
            self.vgg_params[name]['biases'],
            '{}_biases'.format(name)
            )
      }
      return dic
    elif name == 'fc6':
      converted_wt = np.reshape(self.vgg_params[name]['weights'],
                                [7, 7, 512, 4096])
      dic = {
        'weights': tf.Variable(
            converted_wt,
      ...
      dic = {
        'weights': tf.Variable(
            tf.contrib.layers.xavier_initializer()([1, 1, 1000, 21]),
            '{}_weights'.format(name)
            ),
        'biases': tf.Variable(
            tf.zeros([21,]),
      ...
      dic = {
        'weights': tf.Variable(
            tf.contrib.layers.xavier_initializer()([1, 1, 512, 21]),
            '{}_weights'.format(name)
            ),
        'biases': tf.Variable(
            tf.zeros([21,]),
      ...
      dic = {
        'weights': tf.Variable(
            tf.contrib.layers.xavier_initializer()([1, 1, 256, 21]),
            '{}_weights'.format(name)
            ),
        'biases': tf.Variable(
            tf.zeros([21,]),
  ...
  def _get_upconv_params(self, factor, out_channels, name):
    ...
    weights = np.zeros([kernel_sz, kernel_sz, out_channels, out_channels],
                       dtype=np.float32)
  ...
  def _get_loss_layer(self, prediction, annotation, class_weights = None):
    ...
    if class_weights is not None:
      for i in range(21):
        weighted_loss = tf.where(tf.equal(annotation, i),
                                 class_weights[i] * loss_no_nans,
                                 loss_no_nans)
      return tf.reduce_mean(weighted_loss)
    return tf.reduce_mean(loss_no_nans)

  def _get_loss_layer_v2(self, prediction, annotation, class_weights=None):
    one_hot_annotation = tf.one_hot(annotation, 22, on_value=1.0,
                                    off_value=0.0, axis=-1) # (1, H, W, 21)
    one_hot_annotation = tf.reshape(
        tf.slice(
            one_hot_annotation,
            [0, 0, 0, 0],
            [
              ...
              tf.shape(one_hot_annotation)[3]-1
            ]
          ), [-1, 21]) # (H*W, 21)
    if class_weights is not None:
    ...
    weighted_annotation = tf.transpose(
        tf.matmul(one_hot_annotation, tf.transpose(class_weights))
        ) # (1, H*W)
    loss = tf.multiply(weighted_annotation,
                       tf.reshape(
                         tf.nn.softmax_cross_entropy_with_logits(
                           labels = one_hot_annotation,
                           logits = prediction
                         ),
                         [1, -1]
                       ))
  ...
  def build_model(self):
    ...
    self.net['score_fc'] = self._get_conv_layer(self.net['fc8'],
                                                self.params['score_fc'])

    # Upconv 2x
    self.params['upconv2'] = self._get_upconv_params(2, 21, 'upconv2')
    self.net['upconv2'] = tf.nn.conv2d_transpose(
        self.net['score_fc'],
        self.params['upconv2']['weights'],
        output_shape=[
          1,
          tf.shape(self.net['score_fc'])[1] * 2,
          tf.shape(self.net['score_fc'])[2] * 2,
          21
        ],
        strides = [1,2,2,1]
      )
    self.net['cropped_upconv2'] = self._get_crop_layer(self.net['upconv2'],
                                                       self.net['pool4'])

    # Score pool4
    self.params['score_pool4'] = self._get_vgg_conv_params('score_pool4')
    self.net['score_pool4'] = self._get_conv_layer(self.net['pool4'],
                                                   self.params['score_pool4'])

    # Fuse pool4
    self.net['fuse_pool4'] = tf.add(self.net['score_pool4'],
                                    self.net['cropped_upconv2'])

    # Upconv 4x
    self.params['upconv4'] = self._get_upconv_params(2, 21, 'upconv4')
    self.net['upconv4'] = tf.nn.conv2d_transpose(
        self.net['fuse_pool4'],
        self.params['upconv4']['weights'],
        output_shape=[
          1,
          tf.shape(self.net['fuse_pool4'])[1] * 2,
          tf.shape(self.net['fuse_pool4'])[2] * 2,
          21
        ],
        strides = [1,2,2,1]
      )
    self.net['cropped_upconv4'] = self._get_crop_layer(self.net['upconv4'],
                                                       self.net['pool3'])

    # Score pool3
    self.params['score_pool3'] = self._get_vgg_conv_params('score_pool3')
    self.net['score_pool3'] = self._get_conv_layer(self.net['pool3'],
                                                   self.params['score_pool3'])

    # Fuse pool3
    self.net['fuse_pool3'] = tf.add(self.net['score_pool3'],
                                    self.net['cropped_upconv4'])

    # Final score layer
    self.params['scores_final'] = self._get_upconv_params(8, 21,
                                                          'scores_final')
    self.net['scores_final'] = tf.nn.conv2d_transpose(
        self.net['fuse_pool3'],
        self.params['scores_final']['weights'],
        output_shape=[
          1,
          tf.shape(self.net['fuse_pool3'])[1] * 8,
          tf.shape(self.net['fuse_pool3'])[2] * 8,
          21
        ],
        strides=[1,8,8,1]
      )
    self.net['cropped_scores_final'] = self._get_crop_layer(
        self.net['scores_final'],
        self.net['input']
      )

    class_weights = tf.constant(self.class_weights_)

    self.net['loss'] = self._get_loss_layer_v2(
        self.net['cropped_scores_final'],
        self.net['annotation']
      )

  def train(self, max_iterations_coarse, max_iterations_fine,
            save_params_after=None, restore_params=None):
    self.optimizer = tf.train.GradientDescentOptimizer(1e-3)

    coarse_train_step = self.optimizer.minimize(
        self.net['loss'],
        var_list = [
          self.params['score_pool3']['weights'],
          self.params['score_pool4']['weights'],
          self.params['score_fc']['weights']
        ]
      )
    ...
    for iteration in range(max_iterations_coarse):
      ...
      cumul_loss += loss
      if (save_params_after is not None and
          (iteration + 1) % save_params_after == 0):
        cumul_loss /= save_params_after
        self.logger.debug('Iteration #{} average train loss: '
                          '{}'.format(iteration, cumul_loss))
        # Check if the cumulative loss is better than the best loss
        if best_loss is None:
          best_loss = cumul_loss
        if cumul_loss <= best_loss:
          self.logger.debug('Found best coarse params! '
                            'Saving to best_params_coarse/')
          os.system('rm -rf best_params_coarse')
          os.system('mkdir best_params_coarse')
          save_path = coarse_saver.save(self.sess, 'best_params_coarse/'
                                        'fcn_{}.ckpt'.format(iteration+1))
    ...
    for iteration in range(max_iterations_fine):
      ...
      if (save_params_after is not None and
        ...
        if best_loss is None:
          best_loss = cumul_loss
        if cumul_loss <= best_loss:
          self.logger.debug('Found best fine params! '
                            'Saving to best_params_fine/')
          os.system('rm -rf best_params_fine')
          os.system('mkdir best_params_fine')
          save_path = fine_saver.save(self.sess, 'best_params_fine/'
                                      'fcn_{}.ckpt'.format(iteration+1))
          self.logger.debug('Model params after {} iterations saved to {}'
                           .format(iteration+1, save_path))
          best_loss = cumul_loss
